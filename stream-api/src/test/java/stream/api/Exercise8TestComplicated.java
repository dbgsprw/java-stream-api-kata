...
public class Exercise8TestComplicated extends ClassicOnlineStore {
    ...
    public void havingEnoughMoney() {
        ...
        ToIntFunction<Item> lowestPrice = item -> onSale.stream()
                .filter(sale -> sale.getName().equals(item.getName()))
                .mapToInt(Item::getPrice).min().orElse(0);

        Predicate<Customer> havingEnoughMoney = customer ->
                customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(lowestPrice).sum();
